...
class UserApi {
  ...
  Future<Response> userUpdateWithHttpInfo(UpdateInfoCmd updateInfoCmd,) async {
    // ignore: prefer_const_declarations
  ...
  ///
  /// * [UpdateInfoCmd] updateInfoCmd (required):
  Future<bool?> userUpdate(UpdateInfoCmd updateInfoCmd,) async {
    final response = await userUpdateWithHttpInfo(updateInfoCmd,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
