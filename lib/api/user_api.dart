...
class UserApi {
  ...
  /// Performs an HTTP 'POST /api/User/RefreshToken' operation and returns the [Response].
  /// Parameters:
  ///
  /// * [Object] body (required):
  Future<Response> userRefreshTokenWithHttpInfo(Object body,) async {
    // ignore: prefer_const_declarations
    final path = r'/api/User/RefreshToken';
  ...
  ///
  /// * [Object] body (required):
  Future<String?> userRefreshToken(Object body,) async {
    final response = await userRefreshTokenWithHttpInfo(body,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
